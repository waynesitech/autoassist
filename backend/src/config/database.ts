...
export async function initializeDatabase(): Promise<void> {
  try {
    ...
    // Legacy DBs: transactions created without user_id
    try {
    ...
    try {
      await pool.query('ALTER TABLE quotations ADD COLUMN admin_message TEXT');
    } catch (error: any) {
      // Column already exists, ignore error
      if (!error.message?.includes('Duplicate column name')) {
        console.warn('Could not add admin_message column:', error.message);
      }
    }
    ...
    const [workshopRows] = await pool.query('SELECT COUNT(*) as count FROM workshops') as any;
    ...
    // Insert default admin if admin table is empty
    const [adminRows] = await pool.query('SELECT COUNT(*) as count FROM admin') as any;
    if (adminRows[0].count === 0) {
      const hashedPassword = await bcrypt.hash('admin123', 10);
      await pool.query(`
        INSERT INTO admin (email, password, name, role) VALUES
        (?, ?, ?, ?)
      `, ['admin@autoassist.com', hashedPassword, 'Admin User', 'admin']);
    }
