...
export async function initializeDatabase() {
  try {
    ...
    const [transactionRows] = await pool.query('SELECT COUNT(*) as count FROM transactions');
    ...
    const [bannerRows] = await pool.query('SELECT COUNT(*) as count FROM banner_sliders');
    ...
    if (userRows[0].count === 0) {
      // Default password: password123 (will be hashed by bcrypt in the API)
      // For now, we'll insert it and let the API handle hashing on first login attempt
      // Or we can use a simple hash for demo purposes
